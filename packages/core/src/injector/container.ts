...
export class NestdContainer {
  ...
  constructor(private readonly config: AppConfig = void 0) {}
  ...
  public addInjectable(injectable: Type<any>, token: string): void {
    if (!this.modules.has(token)) {
      throw new UnknownModuleException()
    }
    const module = this.modules.get(token)
  ...
  public addExportedComponent(exported: Type<any>, token: string): void {
    if (!this.modules.has(token)) {
      throw new UnknownModuleException()
    }
    const module = this.modules.get(token)
